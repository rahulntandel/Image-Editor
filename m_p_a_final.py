...
window.title("Image Editor....[Steganography & DEEPFAKE Detection]")
...
def submit_en(top, msg_entry):
    ...
    try :
        ...
        show_error(2, "Not Enough Space In Image")
        # enable_windows(window)
        return 
    files = [('png', '*.png'),
             ('jpg', '*.jpg')]
...
def submit_de(text_area): 
    ...
    if not failures:
        ...
        show_error(2, "Passward does not satisfy parameters")
        return
    ...
    try:
        ...
        if mssg == None:
            show_error(2, "Wrong Credentials")
            return
    except:
        show_error(0)
        return 
...
def add_recei():
    ...
    add_rec_tv.column("#0")
    add_rec_tv.heading('#0', text='Receivers', anchor='w')
...
def show_res(test_results):
    ...
    for tr in test_results.itertuples():
        with Image.open(os.path.join(path, tr.Filename)) as img:
            ...
            lab_pred = tk.Label(dfres, text = 'Prediction: '+pdc)
            can_lab2.create_window(185, 50, window = pgrs_lab)
